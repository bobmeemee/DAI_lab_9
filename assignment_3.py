...
Original file is located at
    https://colab.research.google.com/drive/10zfvQgqbS3AnAQ7yPrUrZylUHNdiPol7
"""
...
sweep_id = wandb.sweep(sweep_config, project="lab3_test2")
...
def train(config=None):
    with wandb.init(config=config):
        ...
        for epoch in range(config.epochs):
            avg_loss = train_epoch(network, trainloader, optimizer)
            acc = test(network, testloader)
            print(avg_loss)
            print(acc)
            wandb.log({"loss": avg_loss, "epoch": epoch+1})
            wandb.log({"accuracy": acc, "epoch": epoch+1})


def test(network=None, ldr=None):
